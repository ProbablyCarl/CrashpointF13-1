...
/datum/chatmessage/proc/generate_image(text, atom/target, mob/owner, list/extra_classes, lifespan)
	...
	if(isliving(target) && owned_by.prefs.intent_runechat_color)
		var/mob/living/the_speaker = target
		switch(the_speaker.a_intent)
			if(INTENT_HELP)
				target.chat_color = rgb(112, 190, 92)
				target.chat_color_darkened = rgb(112, 190, 92)
			if(INTENT_DISARM)
				target.chat_color = rgb(68, 99, 145)
				target.chat_color_darkened = rgb(68, 99, 145)
			if(INTENT_GRAB)
				target.chat_color = rgb(228, 190, 64)
				target.chat_color_darkened = rgb(228, 190, 64)
			if(INTENT_HARM)
				target.chat_color = rgb(199, 0, 43)
				target.chat_color_darkened = rgb(199, 0, 43)
	else
	...
	// Slaqps a nice black outline on the message
...
/mob/proc/create_chat_message(atom/movable/speaker, datum/language/message_language, raw_message, list/spans, runechat_flags = NONE)
	...
	if(runechat_flags & EMOTE_MESSAGE)
		new /datum/chatmessage(raw_message, speaker, src, list("emote"))
	else
		new /datum/chatmessage(lang_treat(speaker, message_language, raw_message, spans, null, TRUE), speaker, src, spans)


// Tweak these defines to change the available color ranges
#define CM_COLOR_SAT_MIN	0.3
#define CM_COLOR_SAT_MAX	0.5
#define CM_COLOR_LUM_MIN	0.4
#define CM_COLOR_LUM_MAX	0.6
